...
detectionP_df <- as.data.frame(
  data.table::fread(paste0(data.dir.raw, "/GSE80970_MSACF_raw_pvals.txt")))
pheno_df <- readRDS(paste0(data.dir.raw, "GSE80970_pheno.RDS"))

# data management
row.names(detectionP_df) <- detectionP_df$ID_REF
detectionP_df$ID_REF <- NULL
colnames(detectionP_df) <- as.character(pheno_df$geo_accession)
```

```{R, message = FALSE, warning = FALSE, results = 'hide'}
# predict sex with sest package
library(sest)
```

```{R, eval = FALSE}
# Get predicted sex status
estSex <- estimateSex(beta.value = beta_mat, detecP = detectionP_df)
estSex <- estSex$test$predicted

# Compare predicted gender with phenotype gender
realSex <- data.frame(
  sample = pheno_df$geo_accession,
  realSex = pheno_df$`gender:ch1`,
  stringsAsFactors = FALSE
)
realSex$realSex <- ifelse(realSex$realSex == "female", "F", "M")
compareSex <- cbind(realSex, estSex)
...
compareSexComplete$isSame <- sapply(seq_len(nrow(compareSexComplete)), function(row){
  ifelse(compareSexComplete$realSex[row] == compareSexComplete$estSex[row],
         1, 0)
})
table(compareSexComplete$isSame)
```
...
beta_mat <- readRDS(paste0(data.dir.raw, "/GSE80970_assay.RDS"))
nb.samples <- nrow(phenoRaw_df)
nb.female.samples <- sum(phenoRaw_df$gender.ch1 == "female")
nb.male.samples <- sum(phenoRaw_df$gender.ch1 == "male")
```
...
save(beta_mat,
     ...
     pheno_df,
     file = paste0(data.dir.clinical.filter, "/mtsinai_clinical_filtered.rda"))
```

## Probes QC

```{R, include=FALSE, eval=TRUE}
load(paste0(data.dir.clinical.filter, "/mtsinai_clinical_filtered.rda"))
```
...
```{R, include = FALSE, eval = FALSE}
save(betaQN_BMIQ_female, betaQN_BMIQ_male, pheno_df, file = paste0(data.dir.probes.normalization, "/MtSinai_QNBMIQ.rda"))
...
load(paste0(data.dir.probes.normalization, "/MtSinai_QNBMIQ.rda"))
```
...
```{R, include = FALSE, eval = FALSE}
readr::write_csv(d, paste0(data.dir.pca, "/MtSinai_PCs_usingBetas_female.csv"))
```
...
```{R, include = FALSE, eval = FALSE}
readr::write_csv(d, paste0(data.dir.pca, "/MtSinai_PCs_usingBetas_male.csv"))
```
...
save(
  ...
  pheno_female_df,
  pheno_male_df,
  file = paste0(data.dir.pca, "/MtSinai_QNBMIQ_PCfiltered.rda")
)
```
...
save( 
  ...
  df.chrX.probes,
  df.chrY.probes,
  file = file.path(data.dir.table, "/MtSinai_table.rda"))
```
...
load(paste0(data.dir.pca, "/MtSinai_QNBMIQ_PCfiltered.rda"))
```
...
save(
  ...
  file = paste0(data.dir.neuron, "/MtSinai.rda")
)
```
...
```{R, include = FALSE, eval = TRUE}
load(paste0(data.dir.neuron, "/MtSinai.rda"))
```
...
```{R, include = FALSE}
mtsinai_all_stage_linear_df <- all_stage_linear_df
mtsinai_all_stageSexM_linear_df <- all_stageSexM_linear_df
mtsinai_all_ageSexM_linear_df <- all_ageSexM_linear_df
mtsinai_female_stage_linear_df <- female_stage_linear_df
mtsinai_male_stage_linear_df <- male_stage_linear_df

row.names(mtsinai_all_stage_linear_df) <- NULL
row.names(mtsinai_all_stageSexM_linear_df) <- NULL
row.names(mtsinai_all_ageSexM_linear_df) <- NULL
row.names(mtsinai_female_stage_linear_df) <- NULL
row.names(mtsinai_male_stage_linear_df) <- NULL

cohort <- "MTSINAI"
colnames(mtsinai_all_stage_linear_df) <- c(
  "cpg", paste0(cohort, "_", colnames(mtsinai_all_stage_linear_df)[2:5]))
colnames(mtsinai_all_stageSexM_linear_df) <- c(
  "cpg", paste0(cohort, "_", colnames(mtsinai_all_stageSexM_linear_df)[2:5]))
colnames(mtsinai_all_ageSexM_linear_df) <- c(
  "cpg", paste0(cohort, "_", colnames(mtsinai_all_ageSexM_linear_df)[2:5]))
colnames(mtsinai_female_stage_linear_df) <- c(
  "cpg", paste0(cohort, "_", colnames(mtsinai_female_stage_linear_df)[2:5]))
colnames(mtsinai_male_stage_linear_df) <- c(
  "cpg", paste0(cohort, "_", colnames(mtsinai_male_stage_linear_df)[2:5]))
```

```{R, include = FALSE, eval = FALSE}
save(
  mtsinai_all_stage_linear_df,
  mtsinai_all_stageSexM_linear_df,
  mtsinai_all_ageSexM_linear_df,
  mtsinai_female_stage_linear_df,
  mtsinai_male_stage_linear_df,
  file = paste0(data.dir.single.cpg.pval, "/MtSinai_single_cpg.rda")
)
```
