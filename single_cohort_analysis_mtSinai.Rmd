...
data.dir.raw <- "../../coMethDMR_metaAnalysis/code_validation/Meta_analysis_code/DATASETS/MtSinai/step1_download/"
...
https://rpubs.com/tiagochst/Supplemental_AD_MtSinai_dataset

```{R}
pheno_df <- readRDS(
  file.path(data.dir.old.neuron, "pheno141_withNeuronProp_df.RDS")
)
beta_auto_mat <- readRDS(
  file.path(data.dir.old.pca, "MtSinai_QNBMIQ_PCfiltered.RDS")
)
...
beta_mat <- readRDS(file.path(data.dir.raw, "GSE80970_assay.rds"))
...
```{R}
detP <- as.data.frame(
  data.table::fread(file.path(data.dir.raw, "GSE80970_MSACF_raw_pvals.txt"))
)
row.names(detP) <- detP$ID_REF
detP$ID_REF <- NULL
detP <- detP[
  ,match(paste0(pheno_df$subject.id, "AC"), colnames(detP))]
...
save(
  pheno_df,
  beta_mat,
  file = file.path(data.dir.neuron, "MTSINAI_new.rda")
)
```
...
```{R, include = FALSE, eval = TRUE}
load(file.path(data.dir.neuron, "MTSINAI_new.rda"))
```
...
save(
 mval_noCovs_mat,
 pheno_df,
 file = file.path(data.dir.single.cpg.pval, "MTSINAI_mval_noCovs_and_pheno.rda")
)
```
...
```{R, include = FALSE, eval = TRUE}
load(file.path(data.dir.single.cpg.pval, "MTSINAI_mval_noCovs_and_pheno.rda"))
```
...
mtsinai_female_df <- plyr::adply(mval_noCovs_female_mat,1, function(row){
...
mtsinai_female_df$cpg <- as.character(mtsinai_female_df$cpg)
# identical(row.names(mval_noCovs_female_mat), mtsinai_female_df$cpg)
mtsinai_female_df$fdr <- p.adjust(mtsinai_female_df$pValue, method = "fdr")
cohort <- "MTSINAI"
colnames(mtsinai_female_df) <- c(
  "cpg",
  paste0(
    cohort,
    "_",
    colnames(mtsinai_female_df)[2:ncol(mtsinai_female_df)]
  )
)
```
...
mtsinai_male_df <- plyr::adply(mval_noCovs_male_mat,1, function(row){
  sumOneRegion_df <- data.frame(t(row))
...
mtsinai_male_df$cpg <- as.character(mtsinai_male_df$cpg)
# identical(row.names(mval_noCovs_male_mat), mtsinai_male_df$cpg)
mtsinai_male_df$fdr <- p.adjust(mtsinai_male_df$pValue, method = "fdr")
cohort <- "MTSINAI"
colnames(mtsinai_male_df) <- c(
  "cpg",
  paste0(
    cohort,
    "_",
    colnames(mtsinai_male_df)[2:ncol(mtsinai_male_df)]
  )
)
```
...
mtsinai_stageSexMale_df <- plyr::adply(mval_noCovs_mat, 1, function(row){
...
cohort <- "MTSINAI"
colnames(mtsinai_stageSexMale_df) <- c(
  "cpg",
  paste0(cohort, "_", colnames(mtsinai_stageSexMale_df)[-1])
)
```
...
save(
  mtsinai_female_df,
  mtsinai_male_df,
  mtsinai_stageSexMale_df,
  file = file.path(data.dir.single.cpg.pval, "MTSINAI_final_single_cpg_lm.rda")
)
```
...
```{R, include = FALSE}
load(
  file.path(data.dir.single.cpg.pval, "MTSINAI_mval_noCovs_and_pheno.rda")
)
...
save(
 cometh_all,
 file = file.path(data.dir.region, "MTSINAI_region.rda")
)
```
...
```{R, include = FALSE}
load(file.path(data.dir.single.cpg.pval, "MTSINAI_mval_noCovs_and_pheno.rda"))
load(file.path(data.dir.region, "MTSINAI_region.rda"))
```
...
save(
  info_all,
  median_all,
  file = file.path(data.dir.median, "MTSINAI_median.rda")
)
```
...
```{R, include = FALSE}
load(file.path(data.dir.single.cpg.pval, "MTSINAI_mval_noCovs_and_pheno.rda"))
load(file.path(data.dir.median, "MTSINAI_median.rda"))
```
...
mtsinai_female_df <- plyr::adply(median_female, 1, function(row){
...
mtsinai_female_df$fdr <- p.adjust(
  mtsinai_female_df$pValue,
  method = "fdr"
)
colnames(mtsinai_female_df) <- paste0(
  cohort, "_", c("Estimate", "StdErr", "pValue", "fdr"))
mtsinai_female_df <- cbind(
  info_all, mtsinai_female_df)
mtsinai_female_df <- mtsinai_female_df %>%
  arrange(MTSINAI_pValue)
```
...
mtsinai_male_df <- plyr::adply(median_male, 1, function(row){
...
mtsinai_male_df$fdr <- p.adjust(
  mtsinai_male_df$pValue,
  method = "fdr"
)
colnames(mtsinai_male_df) <- paste0(
  cohort, "_", c("Estimate", "StdErr", "pValue", "fdr"))
mtsinai_male_df <- cbind(
  info_all, mtsinai_male_df)
mtsinai_male_df <- mtsinai_male_df %>%
  arrange(MTSINAI_pValue)
```
...
mtsinai_stageSexMale_df <- plyr::adply(median_all, 1, function(row){
...
colnames(mtsinai_stageSexMale_df) <- paste0(
  cohort, "_", colnames(mtsinai_stageSexMale_df)
)

mtsinai_stageSexMale_df <- cbind(info_all, mtsinai_stageSexMale_df)
```
...
save(
  ...
  mtsinai_male_df,
  mtsinai_stageSexMale_df,
  file = file.path(data.dir.region.pval, "MTSINAI_final_region_based_lm.rda")
)
```
...
```{R}
load(file.path(data.dir.single.cpg.pval, "MTSINAI_final_single_cpg_lm.rda"))
...
```{R}
colnames(mtsinai_female_df) <- gsub(
  "MTSINAI_", "", colnames(mtsinai_female_df)
)
mtsinai_female_bacon_df <- estimation_of_inflation(
  data = mtsinai_female_df, df = 88-4)$data.with.inflation
colnames(mtsinai_female_bacon_df) <- c(
  "cpg",
  paste0("MTSINAI_", colnames(mtsinai_female_bacon_df)[-1])
)
```
...
est.inflation <- estimation_of_inflation(
  data = mtsinai_female_df, df = 88-4
)
...
```{R}
colnames(mtsinai_male_df) <- gsub(
  "MTSINAI_", "", colnames(mtsinai_male_df)
)
mtsinai_male_bacon_df <- estimation_of_inflation(
  data = mtsinai_male_df, df = 53-4)$data.with.inflation
colnames(mtsinai_male_bacon_df) <- c(
  "cpg",
  paste0("MTSINAI_", colnames(mtsinai_male_bacon_df)[-1])
)
```
...
```{R}
colnames(mtsinai_stageSexMale_df) <- gsub(
  "MTSINAI_stageSexMale_", "", colnames(mtsinai_stageSexMale_df)
)
mtsinai_stageSexMale_bacon_df <- estimation_of_inflation(
  data = mtsinai_stageSexMale_df, df = 141-7)$data.with.inflation
colnames(mtsinai_stageSexMale_bacon_df) <- c(
  "cpg",
  paste0("MTSINAI_stageSexMale_", colnames(mtsinai_stageSexMale_bacon_df)[-1])
)
```

```{R}
save(
  mtsinai_female_bacon_df,
  mtsinai_male_bacon_df,
  mtsinai_stageSexMale_bacon_df,
  file = file.path(
    data.dir.single.cpg.pval, "MTSINAI_final_single_cpg_lm_bacon.rda"
  )
)
```
...
```{R}
load(file.path(data.dir.region.pval, "MTSINAI_final_region_based_lm.rda"))
...
mtsinai_female_bacon_df <- estimation_of_inflation(
  data = mtsinai_female_df, df = 88-4)$data.with.inflation
colnames(mtsinai_female_bacon_df) <- paste0(
  "MTSINAI_", colnames(mtsinai_female_bacon_df)
)
```
...
mtsinai_male_bacon_df <- estimation_of_inflation(
  data = mtsinai_male_df, df = 53-4)$data.with.inflation
colnames(mtsinai_male_bacon_df) <- paste0(
  "MTSINAI_", colnames(mtsinai_male_bacon_df)
)
```
...
```{R}
colnames(mtsinai_stageSexMale_df) <- gsub(
  "MTSINAI_|stageSexMale_", "", colnames(mtsinai_stageSexMale_df)
)
mtsinai_stageSexMale_bacon_df <- estimation_of_inflation(
  data = mtsinai_stageSexMale_df, df = 141-7)$data.with.inflation
ncols <- ncol(mtsinai_stageSexMale_bacon_df)
colnames(mtsinai_stageSexMale_bacon_df)[5:ncols] <- paste0(
  "stageSexMale_",
  colnames(mtsinai_stageSexMale_bacon_df)[5:ncols]
)
colnames(mtsinai_stageSexMale_bacon_df) <- paste0(
  "MTSINAI_", 
  colnames(mtsinai_stageSexMale_bacon_df)
)
```
...
save(
  ...
  mtsinai_stageSexMale_bacon_df,
  file = file.path(
    data.dir.region.pval, "MTSINAI_final_region_based_lm_bacon.rda"
  )
)
```
